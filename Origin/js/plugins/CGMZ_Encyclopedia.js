 ...
 */
  ...
  CGMZ_Window_EncyclopediaDisplay.prototype.drawBestiary = function() {
      ...
      //this.drawEncyclopediaCenteredText(CGMZ.Encyclopedia.DropsText, this.lineHeight()*9, true);
      this.drawEncyclopediaLeftText(CGMZ.Encyclopedia.DropsText, this.lineHeight()*9, true);
      //let y = this.drawEncyclopediaBestiaryDrops(enemy.dropItems);
      let y = this.drawEncyclopediaBestiaryCustomDrops(enemy.note);
      y = this.drawEncyclopediaMeta(enemy.meta.cgmzdesc, y);
      y = this.drawEncyclopediaBestiarySketchLeft(enemy.battlerHue, enemy.battlerName, y);
  ...
  // Draws Left Text
  //-----------------------------------------------------------------------------
  CGMZ_Window_EncyclopediaDisplay.prototype.drawEncyclopediaLeftText = function(txt, y, useColor) {
      useColor = useColor || false;
      if(useColor) this.changeTextColor(ColorManager.systemColor());
      this.drawText(txt, 0, y, this.contents.width, 'Left');
      this.changeTextColor(ColorManager.normalColor());
  };
  //-----------------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------------
  CGMZ_Window_EncyclopediaDisplay.prototype.drawEncyclopediaBestiaryCustomDrops = function(notes) {
      let width = this.contents.width/2;
      let y = this.lineHeight()*10;
      let notesList = notes.toString().split(/\r\n|\r|\n/g);
      var dropInfo = [];
      for(let i = 0; i<notesList.length;i++){
          let thisDrop = notesList[i];
          if(thisDrop.includes("drops:")){
            var dropRawInfo = thisDrop.substring(thisDrop.indexOf('[')+1,thisDrop.indexOf(']') );
            dropInfo = dropRawInfo.split(',');
            if( dropInfo[0] == 'a'){
                var drop = $dataArmors[dropInfo[1]]
            }
            else if ( dropInfo[0] == 'w'){
                var drop = $dataWeapons[dropInfo[1]];                
            }
            else if  (dropInfo[0] == 'i'){
                var drop = $dataItems[dropInfo[1]]}
            //switch(thisDrop.)
          }
          else
            continue;
...
/*      for(let i =0; i < drops.length; i++) {
  ...
  // Draw dropped items of an enemy - Always used by Bestiary
  // Returns y-value of line past last drop.
  // Left Justifies Image
  //-----------------------------------------------------------------------------
  CGMZ_Window_EncyclopediaDisplay.prototype.drawEncyclopediaBestiarySketchLeft= function(battlerHue, battlerName, y) {
      ...
      this._battlerSprite.x = 0 + this._battlerSprite.width/2;
  ...
  // Draw dropped items of an enemy - Always used by Bestiary
  // Returns y-value of line past last drop.
  //Centers image x axis
  //-----------------------------------------------------------------------------
  CGMZ_Window_EncyclopediaDisplay.prototype.drawEncyclopediaBestiarySketchCentre = function(battlerHue, battlerName, y) {
      let descriptor = CGMZ.Encyclopedia.SketchText + ": ";
      this.changeTextColor(ColorManager.systemColor());
      this.drawText(descriptor, 0, y, this.contents.width, 'left');
      this.changeTextColor(ColorManager.normalColor());
